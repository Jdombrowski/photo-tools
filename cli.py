...
def generate_html_report(df, insights, output_path=None):
    ...
    # Most used equipment
    most_used_camera = df['camera'].mode().iloc[0] if 'camera' in df.columns and not df['camera'].empty else 'N/A'
    most_used_lens = df['lens'].mode().iloc[0] if 'lens' in df.columns and not df['lens'].empty else 'N/A'
    ...
    camera_rows = generate_table_rows(df['camera'], total_photos) if 'camera' in df.columns else '<tr><td colspan="3">No camera data</td></tr>'
    lens_rows = generate_table_rows(df['lens'], total_photos) if 'lens' in df.columns else '<tr><td colspan="3">No lens data</td></tr>'
    ...
    if 'datetime' in df.columns and not df['datetime'].isna().all():
        ...
        shooting_patterns = '<div class="insight-box">No datetime data available for pattern analysis</div>'
...
def print_summary_report(df, insights):
    ...
    print(f"   Date Range: {df['datetime'].min().strftime('%Y-%m-%d') if 'datetime' in df.columns and not df['datetime'].isna().all() else 'N/A'} to {df['datetime'].max().strftime('%Y-%m-%d') if 'datetime' in df.columns and not df['datetime'].isna().all() else 'N/A'}")
    print(f"   GPS Tagged: {len(df.dropna(subset=['latitude', 'longitude']))} ({(len(df.dropna(subset=['latitude', 'longitude'])) / len(df) * 100):.1f}%)" if 'latitude' in df.columns else 'GPS Tagged: 0')
    ...
    if 'aperture' in df.columns:
        ...
        print(f"   Top Apertures: {', '.join([f'{ap} ({count})' for ap, count in aperture_counts.items()])}")
    ...
    if 'datetime' in df.columns and not df['datetime'].isna().all():
        ...
        print(f"   Most Active Month: {df_temp['datetime'].dt.month_name().mode().iloc[0] if not df_temp['datetime'].dt.month_name().empty else 'N/A'}")
...
def main():
    ...
    parser.add_argument("--format", "-f", choices=['json', 'csv', 'html'], default='console',
    ...
    try:
        ...
        if args.format == 'console':
            ...
            output_file = args.output or f"photo_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            ...
            output_file = args.output or f"photo_metadata_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            df.to_csv(output_file, index=False)
            print(f"📊 CSV data saved to: {output_file}")
            
        elif args.format == 'html' or args.output:
            output_file = args.output or f"photo_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
